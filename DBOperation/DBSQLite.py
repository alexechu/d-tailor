...
class DBSQLite(DBAbstract):
    ...
    def DBInit(self):
        ...
        features_fields = ''.join(
            [feature + "Level TEXT, " for feature in self.designMethod.featuresList])
        ...
        features_values_fields = ''.join([feature +
                                          " " +
        ...
        all_comb = [tuple(d_sol.split('.')) + (d_sol,)
    ...
    def registerWorker(self):
        ...
        for row in self.cur.fetchall():
            ...
            self.des_solutions[key] = {
                'status': str(
                    row['status']), 'des_solution_id': str(
                    row['des_solution_id'])}

        self.cur.execute(
            "select generated_solution_id from generated_solution")
    ...
    def DBGetSolution(self, solution_id):
        ...
        self.cur.execute(
            "select * from generated_solution where generated_solution_id=?", (solution_id,))
    ...
    def DBGetDesiredSolution(self):
        ...
        self.cur.execute(
            "select * from desired_solution where status='WAITING' order by random() LIMIT 1")
        des_solution = self.cur.fetchone()

        if des_solution is not None:
    ...
    def DBChangeStatusDesiredSolution(
            self, desired_solution_id, status='WAITING'):
        self.cur.execute(
    ...
    def DBGetClosestSolution(self, desiredSolution):
        ...
        #fields = '+'.join(["ABS(2*("+feature+"Level - :"+feature+"Level) + "+feature+"Position)" for feature in self.designMethod.features])
        fields = '+'.join(["ABS(2*(:" +
                           feature +
                           "Level - " +
                           feature +
                           "Level) - " +
                           feature +
                           "Position)" for feature in self.designMethod.features])

        query = "SELECT generated_solution_id, sequence FROM generated_solution ORDER BY (" + \
            fields + ") limit 10;"
    ...
    def DBInsertSolution(self, solution, desired_solution_id=""):
        ...
        key = '.'.join([str(solution.levels[feature + 'Level'])
                        for feature in self.designMethod.featuresList])

        if not self.designMethod.listDesigns == [
        ]:  # RandomSampling mode does not have desired targets
            if desired_solution_id == "":  # Worker found solution for something it WASN't looking for
                ...
                if key in self.des_solutions:
                    desired_solution_id = str(
                        self.des_solutions[key]['des_solution_id'])
        ...
        dict_with_values.update(
            {
                (
                    feature +
                    'Position'): self.calculateRelativeScore(
                    feature,
                    solution.levels[
                        feature +
                        'Level'],
                    solution.scores[feature]) for feature in self.designMethod.features})
    ...
    def calculateRelativeScore(self, feature="", level=1, featureScore=0):
        ...
        else:  # anything else other than a tuple with lower and upper limits
            return 0
