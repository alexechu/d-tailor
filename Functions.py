...
def complementary(seq):
    ...
    """
    return str(seq).translate(str.maketrans("atcg", "tagc"))
...
def structureAnalysis(structure_file, propertyOfInterest="ss"):
    ...
    if not os.path.exists(filepath):
        print("structureAnalysis: %s Not Found" % filepath,
              file=sys.stderr)
        return []

    if propertyOfInterest == "ds":
        output = subprocess.Popen(
            "perl 3rdParty/unafold/ss-count.pl " +
            filepath +
            " | awk /[[:digit:]][[:blank:]]0/'{print $1}'",
...
def analyze_terminator(seq, debug=False):
    max_term_score = look_for_terminators(seq=seq, debug=debug)

    if max_term_score > 90:
        return 'Yes'
    elif max_term_score > 70:
        return 'Maybe'
    else:
        return 'No'
...
def analyze_duplex_structure(seq1, seq2, filename, debug=False,
    unafolddat=Data.project_dir + "/3rdParty/unafold/data"):
    os.chdir(Data.project_dir)

    data = {'RNADuplexPairs': {}}

    seq1_file = tempfile.NamedTemporaryFile(mode='w+', delete=False,
        dir=Data.project_dir + "tmp/unafold/")
    seq2_filename = seq1_file.name + ".fa2"

    print(seq1, file=seq1_file)
    seq1_file.close()
    with open(seq2_filename, "w") as fh:
        print(seq2, file=fh)

    os.environ["UNAFOLDDAT"] = unafolddat
    with open(os.devnull, "w+") as dnull:
        subprocess.check_call(["./3rdParty/unafold/hybrid-min",
                         "-n", "RNA",
                         "-o", filename,
                         seq1_file.name, seq2_filename,
                         "-t", "37", "-T", "37"],
                         stdout=dnull,
                         stderr=dnull,
                         universal_newlines=True)

    pairs_df = pd.read_csv("%s.ct" % filename,
                            sep="\t",
                            header=None,
                            skiprows=1,
                            usecols=[0, 4])

    for base1, base2 in pairs_df.itertuples(index=False, name=None):
        data['RNADuplexPairs'][base1] = base2

    for fn in glob.glob("%s*.ct" % filename):
        os.rename(fn, "tmp/structures/" + fn)

    for fn in glob.glob("%s*.asc" % filename):
    ...
    if not debug:
        ...
        for fn in glob.glob("%s*.run" % filename):
            os.remove(fn)

        for fn in glob.glob("%s*.ext" % filename):
            os.remove(fn)

        for fn in glob.glob("%s*.plot" % filename):
            os.remove(fn)

    return data


def get_unafold_mfe(filename, region=None):
    """
        Return the deltaG of the .ct file provided
    """
    ...
    if region is None:
        ...
        try:
            # this is the deltaG
            return float(header.split()[3])
        except ValueError:
            print("get_unafold_mfe: Unable to parse %s" % filename,
                  file=sys.stderr)
            return 'NA'
...
def analyze_duplex_mfe(filename, region=None):
    ct_filename = "%s/tmp/structures/%s.ct" % (Data.project_dir, filename)
    return {'RNADuplexMFE': get_unafold_mfe(ct_filename)}
...
def analyze_duplex_ds(filename, seq1="", region=None):
    ...
    filepath = Data.project_dir + "/tmp/structures/" + filename + ".ct"
    ...
    if region is None:
        if os.path.exists(filepath):
            ...
            print("analyze_duplex_ds: %s not found" % filepath)
...
def analyze_duplex_ss(filename, seq1="", region=None):
    ...
    if region is None:
        if os.path.exists(Data.project_dir + "/tmp/structures/" + filename + ".ct"):
            ...
            data['RNADuplexSingleStrandedBases_Mol2'] = 'NA'
    else:
        # TODO: get just DS bases from one region of the rna structure
        pass
...
def analyze_structure_prob(seq, filename, window=50, region=[], debug=False):
    data = {'StructureProbList': {}}

    temp_name = tempfile.NamedTemporaryFile(mode='w+', dir="").name

    seq_input = ">" + temp_name + "\n" + seq + "\n"

    _ = subprocess.check_output(
        [Data.project_dir + '3rdParty/vienna/RNAplfold',
         "-d2", "-noLP",
         "-W", str(window),
         "-u", "1"],
         input=seq_input,
    ...
    unpaired_probs_df = pd.read_csv(temp_name + "_lunp",
                                    header=None,
                                    sep="\t",
                                    skiprows=2,
                                    usecols=[0, 1],
                                    names=['pos', 'prob'])
    print(unpaired_probs_df)

    for pos, prob in unpaired_probs_df.itertuples(index=False, name=None):
        data['StructureProbList'][pos] = prob

    if region != []:
        reg_avg = 0
        for pos in region:
            reg_avg += data['StructureProbList'][str(pos)]
        reg_avg = reg_avg / len(region)
        data['StructureProb'] = reg_avg
    else:
        data['StructureProb'] = unpaired_probs_df['prob'].mean()

    if not debug:
        for fn in glob.glob("%s*" % temp_name):
            os.remove(fn)
...
def analyze_structure(seq, filename, ensemble=False, debug=False,
    ...
    if ensemble:
        exec_call = "./3rdParty/unafold/UNAFold.pl"
    else:
        exec_call = "./3rdParty/unafold/hybrid-ss-min"

    os.environ["UNAFOLDDAT"] = unafolddat

    with open(os.devnull, 'w') as dnull:
        subprocess.check_call([exec_call, "-n", "RNA", filename + ".seq"],
                              stdout=dnull,
                              stderr=dnull)
...
def analyze_structure_mfe(filename, region=None):
    ct_filename = "%s/tmp/structures/%s.ct" % (Data.project_dir, filename)
    return {'StructureMFE': get_unafold_mfe(ct_filename)}
...
def look_for_terminators(seq, debug=False):
    # use transtermHP
    # create input files
    file1 = tempfile.NamedTemporaryFile(mode='w+', delete=False,
        suffix='.fa', dir=Data.project_dir + "tmp/transterm_files/")
    file2 = tempfile.NamedTemporaryFile(mode='w+', delete=False,
        suffix='-fake.coords', dir=Data.project_dir + "tmp/transterm_files/")

    print(">seq1", file=file1)
    print(seq, file=file1)
    file1.close()

    print("fakegene1    1 2    seq1", file=file2)
    print("fakegene1    " + str(len(seq) - 1) + " " + str(len(seq)) + "    seq1", file=file2)
    file2.close()

    # run transtermhp
    with open(os.devnull, 'w') as dnull:
        output = subprocess.check_output(
            ["./3rdParty/transterm/transterm",
            "-p", "./3rdParty/transterm/expterm.dat",
    ...
    for line in output.decode().split('\n'):
        if line.startswith('  TERM'):
            current_score = float(line.split()[7])
