...
def structureAnalysis(structure_file, propertyOfInterest="ss"):
    ...
    if not os.path.exists(structure_file + ".ct"):
        print("structureAnalysis: %s.ct Not Found", file=sys.stderr)
        return []
...
def analyze_duplex_structure(seq1, seq2, filename, debug=False,
    ...
    # redirecting to os.devnull is necessary to omit output generated by UNAFOLD
    with open(os.devnull, 'w') as fnull:
        os.environ["UNAFOLDDAT"] = unafolddat
        subprocess.call(["./3rdParty/unafold/hybrid-min",
                         "-n", "RNA",
                         "-o", filename,
                         s1_file, s2_file,
                         "-t", "37", "-T", "37"],
        ...
        if os.path.exists(Data.project_dir + "/" + filename + ".ct"):
            ...
            pairs_df = pd.read_csv("tmp/structures/%s.ct" % filename,
                                    sep="\t",
                                    header=None,
                                    skiprows=1,
                                    usecols=[0, 4])
...
def analyze_duplex_mfe(filename, region=None,
    ...
    if region is None:
        ...
        res = subprocess.check_output(
            [Data.project_dir + "/3rdParty/unafold/ct-energy", ct_filename])

        try:
            data['RNADuplexMFE'] = float(res)
        except ValueError:
            data['RNADuplexMFE'] = 'NA'
