...
class StructureEnsemble(Feature):
    ...
    def mutate(self, operator=Functions.SimpleStructureOperator):        
        if not self.targetInstructions:
            return None        
        ss_bases = None if self.label+'StructureSingleStrandedBasesList' not in self.scores else self.scores[self.label+'StructureSingleStrandedBasesList']
        ds_bases = None if self.label+'StructureDoubleStrandedBasesList' not in self.scores else self.scores[self.label+'StructureDoubleStrandedBasesList']                    
