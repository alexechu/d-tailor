...
class RNADuplex(Features.Feature.Feature):
    ...
    def __init__(self, duplexObject=None, solution1=None, solution2=None,
        ...
        if duplexObject is None:
            ...
            self.duplexFile = str(solution1.solid) + '-' + \
                str(solution2.solid) + label
            ...
            self.mutable_region = args[
                'mutable_region1'] if 'mutable_region1' in args else None
            self.cds_regions = args[
                'cds_regions1'] if 'cds_regions1' in args else None
            ...
            self.mutable_region2 = args[
                'mutable_region2'] if 'mutable_region2' in args else None
            self.cds_regions2 = args[
                'cds_regions2'] if 'cds_regions2' in args else None
    ...
    def set_scores(self, scoring_function=Functions.analyze_duplex_structure):
        self.scores.update(
            Functions.appendLabelToDict(
                scoring_function(
                    self.rnaMolecule1seq,
                    self.rnaMolecule2seq,
                    self.duplexFile),
                self.label))
        pass
...
class RNADuplexRibosome(RNADuplex):
    ...
    def __init__(self, solution1=None, label="", args={'rnaMolecule1region': None,
        ...
        self.solution = solution1
        self.keep_aa = args[
            'keep_aa'] if 'keep_aa' in args else solution1.keep_aa
        self.mutable_region = args[
            'mutable_region'] if 'mutable_region' in args else solution1.mutable_region
        self.cds_regions = args[
            'cds_regions'] if 'cds_regions' in args else solution1.mutable_region
...
class RNADuplexMFE(RNADuplex):
    ...
    def __init__(self, duplexObject, label="", regionOfInterest=None):
