...
class Bottleneck(Features.Feature.Feature):
    ...
    def calculate_mutable_segments(self):
        ...
        for level in list(self.solution.designMethod.thresholds[
                          self.label + "BottleneckPosition"].keys()):
            segment = self.solution.designMethod.thresholds[
    ...
    def mutate(self, operator=Functions.SimpleBneckOperator):
        ...
        # check codons available for mutation
        mutableCodonsPosition = self.segmentMutation[
...
class BottleneckRelativeStrength(Bottleneck):
    ...
    def defineTarget(self, desiredSolution):
        ...
        target_level_for_strength = desiredSolution[
            self.label + 'BottleneckRelativeStrengthLevel']
        bneck_position_level = self.parentFeature.subfeatures[
            self.label + 'BottleneckPosition'].level
