...
class PWMScore(Feature):
    ...
    def __init__(self, solution=None, label="", args = { 'pwm' : None,
        ...
        self.sequence           = solution.sequence[self.pwm_range[0]:self.pwm_range[1]+1]
        self.mutable_region     = [i-self.pwm_range[0] for i in set(range(self.pwm_range[0],self.pwm_range[1]+1)) & set(args['mutable_region'])] if args.has_key('mutable_region') else solution.mutable_region
    ...
    def mutate(self, operator=Functions.SimplePWMScoreOperator):
        ...
        if mutated_seq == None:
            ...
            new_seq[self.ntusage_range[0]:self.ntusage_range[1]+1] = list(mutated_seq)
        new_seq = "".join(new_seq) 
                
        return Solution.Solution(sol_id=str(uuid4().int), sequence=new_seq, cds_regions = self.cds_regions, mutable_region = self.mutable_region, parent=self.solution, design=self.solution.designMethod)
